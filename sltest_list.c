...
hash function
declared nlist
list is now an array. initialized array.
when inserting, compute hash and insert.

int index = hash(elements[j].key);
SortedList_insert(&list[index], &elements[j]);
same with lookup
 */
...
void* threadfunc(void* arg) {
	...
	for (int i = 0; i < iterations; i++) {
		...
		int list_hash = hash(elements[index].key);
		if (mutex) {
			// use pthread_mutex
		  fprintf(stderr, "insert = mutex\n");
		  pthread_mutex_lock(&lock);
			SortedList_insert(&list[list_hash], &elements[index]);
...
void sltest(long nthreads, long niter, char opt_yield) {
	// malloc list array
        list = malloc(nlists*sizeof(SortedList_t));
	if(list == NULL) {
